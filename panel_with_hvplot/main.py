...
pipeline = avg.hvplot(height=300, width=400, color="blue", legend=False) * avg[
    outliers
].hvplot.scatter(color="orange", padding=0.1, legend=False)
...
# Servable App
pn.Column(pipeline.widgets(), pn.Row(count.output(), pipeline.output())).servable(
    target="panel"
)
