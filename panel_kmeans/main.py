...
x = pn.widgets.Select(name="x", options=cols, value="bill_depth_mm").servable(
    target="x-widget"
)
y = pn.widgets.Select(name="y", options=cols, value="bill_length_mm").servable(
    target="y-widget"
)
n_clusters = pn.widgets.IntSlider(name="n_clusters", start=1, end=5, value=3).servable(
    target="n-widget"
)
...
def get_chart(x, y, df):
    ...
    return alt.Chart(df).mark_point(size=100).encode(
        ...
        shape="labels",
        color="species",
    ).add_params(brush).properties(width=800) + alt.Chart(centers).mark_point(
        size=250, shape="cross", color="black"
    ).encode(
        x=x + ":Q", y=y + ":Q"
    )
...
chart = pn.pane.Vega(debounce=10).servable(target="cluster-plot")
...
@param.depends(chart.selection.param.brush, watch=True)
